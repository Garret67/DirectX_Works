...
HRESULT InitEffect(void)
{
	...
	for (int i = 0; i < EFFECT_NUM_EFFECTS; i++)
	{
		...
		for (int n = 0; n < EFFECT_NUM_PARTICLES; n++)
		{
			ResetParticle(i, n);
		}
	}
	...
	for (int i = 0; i < EFFECT_NUM_EFFECTS; i++)
	{
		if (effectWk[i].use)
		{
			...
			if (effectWk[i].isRemoveOnFinish == FALSE)
			{
				...
				//エフェクト作成レートの増加処理
				if (effectWk[i].effectCount < EFFECT_NUM_PARTICLES)
					effectWk[i].emitCounter++;

				//バッファに空きがあり、追加タイミングが来ていれば新たなエフェクトを追加する
				while ((effectWk[i].effectCount < EFFECT_NUM_PARTICLES) && (effectWk[i].emitCounter > EMISSION_RATE))
				{
					//全体追加フラグがONであれば空き領域全てに新たなエフェクトを追加する
					if (EMISSION_FULL)
						effectWk[i].effectCount = EFFECT_NUM_PARTICLES;
					else
