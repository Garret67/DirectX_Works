...
// game pad用設定値
#define DEADZONE		500/*2500*/			// 各軸の25%を無効ゾーンとする
...
void UpdatePad(void)
{
	...
	for ( i=0 ; i<padCount ; i++ ) 
	{
		...
		//* y-axis (forward)
		if ( dijs.lY < 0 )					padState[i] |= BUTTON_JS_UP;
		//* y-axis (backward)
		if ( dijs.lY > 0 )					padState[i] |= BUTTON_JS_DOWN;
		//* x-axis (left)
		if ( dijs.lX < 0 )					padState[i] |= BUTTON_JS_LEFT;
		//* x-axis (right)
		if ( dijs.lX > 0 )					padState[i] |= BUTTON_JS_RIGHT;
		//* Ｘボタン
		...
		if (dijs.rgdwPOV[0] != 0xFFFFFFFF)
		{
			...
			switch (dijs.rgdwPOV[0])
			{
				// 上
			case 0:
				padState[i] |= BUTTON_ARROW_UP;
				break;
				// 右上
			case 4500:
				padState[i] |= BUTTON_ARROW_UP;
				padState[i] |= BUTTON_ARROW_RIGHT;
				break;
				// 右
			case 9000:
				padState[i] |= BUTTON_ARROW_RIGHT;
		...
		//PrintDebugProc("JSLeft: X:%ld Y:%ld \n", dijs.lX, dijs.lY);
		//PrintDebugProc("Otro:   Z:%ld lRz:%ld \n", dijs.lZ, dijs.lRz);
		PrintDebugProc("JSLeft: X: %d Y: %d \n", static_cast<int>(dijs.lX), static_cast<int>(dijs.lY));



		//#define DEADZONE_THRESHOLD_X DEADZONE
		//#define DEADZONE_THRESHOLD_Y DEADZONE
		//
		//		// Obtener la configuracion actual de la zona muerta para el eje X
		//		DIPROPDWORD dipdwX;
		//		dipdwX.diph.dwSize = sizeof(DIPROPDWORD);
		//		dipdwX.diph.dwHeaderSize = sizeof(DIPROPHEADER);
		//		dipdwX.diph.dwHow = DIPH_DEVICE;
		//		dipdwX.diph.dwObj = DIJOFS_X;
		//		pGamePad[i]->GetProperty(DIPROP_DEADZONE, &dipdwX.diph);
		//
		//		// Obtener la configuracion actual de la zona muerta para el eje Y
		//		DIPROPDWORD dipdwY;
		//		dipdwY.diph.dwSize = sizeof(DIPROPDWORD);
		//		dipdwY.diph.dwHeaderSize = sizeof(DIPROPHEADER);
		//		dipdwY.diph.dwHow = DIPH_DEVICE;
		//		dipdwY.diph.dwObj = DIJOFS_Y;
		//		pGamePad[i]->GetProperty(DIPROP_DEADZONE, &dipdwY.diph);
		//
		//		// Ajuste dinamico de la zona muerta para el eje X en funcion del eje Y
		//		if (abs(dijs.lY) > DEADZONE_THRESHOLD_Y) {
		//			dipdwX.dwData = 0;  // No se aplica la zona muerta para el eje X
		//			dipdwX.diph.dwObj = DIJOFS_X;
		//			pGamePad[i]->SetProperty(DIPROP_DEADZONE, &dipdwX.diph);
		//		}
		//		else {
		//			// Aplicar la zona muerta normal para el eje X
		//			dipdwX.dwData = DEADZONE;
		//			dipdwX.diph.dwObj = DIJOFS_X;
		//			pGamePad[i]->SetProperty(DIPROP_DEADZONE, &dipdwX.diph);
		//		}
		//
		//		// Ajuste dinamico de la zona muerta para el eje Y en funcion del eje X
		//		if (abs(dijs.lX) > DEADZONE_THRESHOLD_X) {
		//			dipdwY.dwData = 0;  // No se aplica la zona muerta para el eje Y
		//			dipdwY.diph.dwObj = DIJOFS_Y;
		//			pGamePad[i]->SetProperty(DIPROP_DEADZONE, &dipdwY.diph);
		//		}
		//		else {
		//			// Aplicar la zona muerta normal para el eje Y
		//			dipdwY.dwData = DEADZONE;
		//			dipdwY.diph.dwObj = DIJOFS_Y;
		//			pGamePad[i]->SetProperty(DIPROP_DEADZONE, &dipdwY.diph);
		//		}
...
float GetJoystickLevelX(int padNo)
{
	return dijs.lX / static_cast<float>(RANGE_MAX);
}

float GetJoystickLevelY(int padNo)
{
	return dijs.lY / static_cast<float>(RANGE_MAX);
}

float GetJoystickLevelZ(int padNo)
{
	return dijs.lZ / static_cast<float>(RANGE_MAX);
}

float GetJoystickLevelRz(int padNo)
{
	return dijs.lRz / static_cast<float>(RANGE_MAX);
}
