...
void SetSpriteRightTop(ID3D11Buffer* buf, float X, float Y, float Width, float Height, float U, float V, float UW, float VH)
{
	...
	// Esquina superior derecha como origen
	vertex[0].Position = XMFLOAT3(X - Width, Y, 0.0f); // Cambio en X
	vertex[0].Diffuse = XMFLOAT4(1.0f, 1.0f, 1.0f, 1.0f);
	vertex[0].TexCoord = XMFLOAT2(U, V);

	vertex[1].Position = XMFLOAT3(X, Y, 0.0f); // Cambio en X
	vertex[1].Diffuse = XMFLOAT4(1.0f, 1.0f, 1.0f, 1.0f);
	vertex[1].TexCoord = XMFLOAT2(U + UW, V);

	vertex[2].Position = XMFLOAT3(X - Width, Y + Height, 0.0f); // Cambio en X y Y
	vertex[2].Diffuse = XMFLOAT4(1.0f, 1.0f, 1.0f, 1.0f);
	vertex[2].TexCoord = XMFLOAT2(U, V + VH);

	vertex[3].Position = XMFLOAT3(X, Y + Height, 0.0f); // Cambio en X y Y
	vertex[3].Diffuse = XMFLOAT4(1.0f, 1.0f, 1.0f, 1.0f);
	vertex[3].TexCoord = XMFLOAT2(U + UW, V + VH);
...
// 指定座標を左下として描画する
void SetSpriteLBColor(ID3D11Buffer* buf,
	float X, float Y, float Width, float Height,
	float U, float V, float UW, float VH,
	XMFLOAT4 color)
{
	...
	// 左上を原点として設定するプログラム
	vertex[0].Position = XMFLOAT3(X, Y - Height, 0.0f);
	vertex[0].Diffuse = color;
	vertex[0].TexCoord = XMFLOAT2(U, V);

	vertex[1].Position = XMFLOAT3(X + Width, Y - Height, 0.0f);
	vertex[1].Diffuse = color;
	vertex[1].TexCoord = XMFLOAT2(U + UW, V);
	...
	vertex[3].Position = XMFLOAT3(X + Width, Y, 0.0f);
	vertex[3].Diffuse = color;
	vertex[3].TexCoord = XMFLOAT2(U + UW, V + VH);
