...
BOOL RayCast(XMFLOAT3 xp0, XMFLOAT3 xp1, XMFLOAT3 xp2, XMFLOAT3 xpos0, XMFLOAT3 xpos1, XMFLOAT3 *hit, XMFLOAT3 *normal)
{
	...
	{	// 求めたポリゴンの法線と２つのベクトル（線分の両端とポリゴン上の任意の点）の内積とって衝突している可能性を調べる
		...
		//los multiplica para ver si el signo de los dotproduct son iguales o diferentes
...
//*****************************************************************************
int CheckCollisionAndGetDirection3D(
	float boxAX, float boxAY, float boxAZ, float boxAWidth, float boxAHeight, float boxADepth,
	float boxBX, float boxBY, float boxBZ, float boxBWidth, float boxBHeight, float boxBDepth
)
{
	// 座標の差を計算する
	float dx = boxAX - boxBX;
	float dy = boxAY - boxBY;
	float dz = boxAZ - boxBZ;
	...
	float boxAHalfDepth = boxADepth * 0.5f;
	float boxBHalfDepth = boxBDepth * 0.5f;

	// 幅と高さと奥行きの半分を合計する
	float combinedHalfWidths = boxAHalfWidth + boxBHalfWidth;
	float combinedHalfHeights = boxAHalfHeight + boxBHalfHeight;
	float combinedHalfDepths = boxAHalfDepth + boxBHalfDepth;

	// 横と縦と奥行き方向のあたり判定を確認する
	if (fabsf(dx) < combinedHalfWidths && fabsf(dy) < combinedHalfHeights && fabsf(dz) < combinedHalfDepths)
	{
		float overlapX = combinedHalfWidths - fabsf(dx);
		float overlapY = combinedHalfHeights - fabsf(dy);
		float overlapZ = combinedHalfDepths - fabsf(dz);
