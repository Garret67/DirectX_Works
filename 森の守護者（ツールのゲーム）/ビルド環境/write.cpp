...
void SetFont(const WCHAR* FontName, float fontSize, const D2D1_COLOR_F color, ALIGNMENT alignment)
{
	if (DirectWrite[0].TextFormat != NULL) DirectWrite[0].TextFormat->Release();
	if (DirectWrite[0].FD[0].fontCollection != NULL) DirectWrite[0].FD[0].fontCollection->Release();
	if (DirectWrite[0].SolidBrush != NULL) DirectWrite[0].SolidBrush->Release();


	DirectWrite[0].DWriteFactory[0].CreateTextFormat(FontName, 
													DirectWrite[0].FD[0].fontCollection,
													DirectWrite[0].FD[0].fontWeight, 
													DirectWrite[0].FD[0].fontStyle, 
													DirectWrite[0].FD[0].fontStretch,
													fontSize/* - 5*/, 
													DirectWrite[0].FD[0].localeName, 
													&DirectWrite[0].TextFormat);
	...
	{
	case 0:
		DirectWrite[0].TextFormat[0].SetTextAlignment(DWRITE_TEXT_ALIGNMENT::DWRITE_TEXT_ALIGNMENT_LEADING);//左右ぞろえ
		break;
	case 1:
		DirectWrite[0].TextFormat[0].SetTextAlignment(DWRITE_TEXT_ALIGNMENT::DWRITE_TEXT_ALIGNMENT_CENTER);//左右ぞろえ
		break;
	case 2:
		DirectWrite[0].TextFormat[0].SetTextAlignment(DWRITE_TEXT_ALIGNMENT::DWRITE_TEXT_ALIGNMENT_TRAILING);//左右ぞろえ
		break;
	}
	...
	{
	case 0:
		DirectWrite[0].TextFormat[0].SetParagraphAlignment(DWRITE_PARAGRAPH_ALIGNMENT::DWRITE_PARAGRAPH_ALIGNMENT_NEAR);//上下ぞろえ
		break;
	case 1:
		DirectWrite[0].TextFormat[0].SetParagraphAlignment(DWRITE_PARAGRAPH_ALIGNMENT::DWRITE_PARAGRAPH_ALIGNMENT_CENTER);//上下ぞろえ
		break;
	case 2:
		DirectWrite[0].TextFormat[0].SetParagraphAlignment(DWRITE_PARAGRAPH_ALIGNMENT::DWRITE_PARAGRAPH_ALIGNMENT_FAR);//上下ぞろえ
		break;
	}
}
...
void InitDirectWrite(void)
{
	...
	DW[0].FD->textAlignment = DWRITE_TEXT_ALIGNMENT::/*DWRITE_TEXT_ALIGNMENT_LEADING*/DWRITE_TEXT_ALIGNMENT_CENTER;
...
void UninitDirectWrite(void)
{
	...
	if (DW[0].FD->fontCollection) DW[0].FD->fontCollection->Release();
