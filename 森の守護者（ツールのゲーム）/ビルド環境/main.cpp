...
HRESULT Init(HINSTANCE hInstance, HWND hWnd, BOOL bWindow)
{
	...
	SetLightEnable(TRUE);
	SetCullingMode(CULL_MODE_BACK);	// 背面ポリゴンをカリング

	InitFade();						// フェードの初期化

	SetMode(g_Mode);				// 最初のモードをセット// ここはSetModeのままで！
...
void Update(void)
{
	...
	case MODE_FIGHT:			// 対戦画面の更新
		UpdateGame();
		break;


	case MODE_TUTORIAL:		// リザルト画面の更新
		UpdateTutorial();
		break;
	}
...
void Draw(void)
{
	...
	case MODE_FIGHT:		// 対戦画面の描画
		DrawGame();
		break;

	case MODE_TUTORIAL:		// リザルト画面の描画
		SetViewPort(TYPE_FULL_SCREEN);
...
void SetMode(int mode)
{
	...
	case MODE_GAME:
		UninitCamera();		// カメラもここで初期化しておく事にした
		InitCamera();
