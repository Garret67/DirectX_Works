...
//#endif
...

	{
		...
		D3DX11CreateShaderResourceViewFromFile(GetDevice(),
			"data/TEXTURE/Menu/fractalnoise.jpg",
			NULL,
			NULL,
			&g_Fractalnoise,
			NULL);
	...
	g_ImmediateContext->VSSetConstantBuffers(7, 1, &g_CameraBuffer);//vertex shader
	g_ImmediateContext->PSSetConstantBuffers(7, 1, &g_CameraBuffer);//pixel shader 
...
void DrawTextRect(char* text, int size, XMFLOAT4 color, float left, float top, float right, float bottom, TEXT_ANCHOR anchor, char* textFont)
{
	...
	if (SUCCEEDED(hr))
	{
		...
		if (SUCCEEDED(hr))
		{
			...
			RECT screenRect;
			screenRect.left		= (LONG) left;
			screenRect.top		= (LONG) top;
			screenRect.right	= (LONG) right;
			screenRect.bottom	= (LONG) bottom;
			...
			//Font作成
			HFONT font = CreateFont(size, 0, 0, 0, FW_DONTCARE, FALSE, FALSE, FALSE, SHIFTJIS_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, DEFAULT_QUALITY, FF_DONTCARE, textFont/*"Showcard Gothic"*/);//フォントの指定　引数にできる
			HGDIOBJ hgdi = SelectObject(hdc, font);
			//rect.top += DrawText(hdc, str, -1, &rect, DT_WORDBREAK); //縦がずれる？


			//テキスト出力
			DrawText(hdc, text, (int)strlen(text), &screenRect, rectDT);
